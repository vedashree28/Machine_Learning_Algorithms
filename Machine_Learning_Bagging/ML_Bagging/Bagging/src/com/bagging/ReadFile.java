...
public class ReadFile {
	...
	public static void main(String[] args) {
		...
		int count = 0;
		int no_of_classifiers=100;
		String instances = "";
		try {
			// Use Buffered read to read files
			String trainingSetFileName = System.getProperty("user.dir")
					+ "\\" + args[0];
			BufferedReader reader = new BufferedReader(new FileReader(trainingSetFileName));//training file
			...
			BufferedReader reader1 = new BufferedReader(new FileReader(testSetFileName));//testing file
			...
			no_of_classifiers=Integer.parseInt(args[2]);
			if(no_of_classifiers<=0){
				no_of_classifiers=10;
			}
			...
			System.out.println("Accuracy of <"+ dataIn.size()+"Instances>"+ "with bags "+no_of_classifiers+" :"+accuracy);
			reader.close();
		}
		catch(Exception e){
